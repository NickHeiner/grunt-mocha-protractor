...
module.exports = function (files, browser, options, next) {
  console.log('runner', options.grep);
  var capabilities,
      driver,
  ...
  if (typeof browser === 'string') {
    try {
      ...
      console.log('[error] Unknown browser type: ' + JSON.stringify(browser));
      return next();
    }
  ...
  console.log('driver get session');
  driver.getSession().then(function(session) {
    console.log('driver session acquired');
    ...
    d.on('error', function(err) {
      if (runner) {
        ...
        console.log('[error] Mocha failed to run');
